...
describe Robot do
  ...
  describe "#attack" do
    it "hits enemy with equipped weapon if enemy is a robot" do
      enemy = Robot.new
      expect{@robot.attack(enemy)}.to change{enemy.health}.by(25)
    end

    it "does nothing if enemy is not a robot" do
      enemy = Item.new
      expect{@robot.attack(enemy)}.to_not change{enemy.health}.by(25)
    end
  end
